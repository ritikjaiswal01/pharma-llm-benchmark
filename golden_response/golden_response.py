"""
Golden response generator for the Enterprise Pharmaceutical MIR Platform benchmark.

Run:
    python golden_response.py
    python golden_response.py --write-workspace pharma-mir-platform
    python golden_response.py --output GOLDEN_RESPONSE.md

The file is intentionally self-contained. It can print a complete benchmark answer
or materialize a realistic Next.js + Prisma workspace with the core files needed
for the Medical Information Request platform.
"""
...
WORKSPACE_FILES: dict[str, str] = {
    "package.json": r'''{
  "name": "enterprise-pharma-mir-platform",
  "version": "1.0.0",
  "private": true,
  "scripts": {
    "dev": "next dev",
...
''',
    ".env.example": r'''DATABASE_URL="postgresql://postgres:postgres@localhost:5432/pharma_mir?schema=public"
...
''',
    "prisma/schema.prisma": r'''generator client {
...
''',
    "src/lib/prisma.ts": r'''import { PrismaClient } from "@prisma/client";
...
''',
    "src/lib/security.ts": r'''import crypto from "crypto";
...
''',
    "src/lib/validation.ts": r'''import validator from "validator";
...
''',
    "src/lib/rate-limit.ts": r'''import { prisma } from "./prisma";
...
''',
    "src/lib/email.ts": r'''import nodemailer from "nodemailer";
...
''',
    "src/app/api/medical-inquiry/route.ts": r'''import { NextRequest, NextResponse } from "next/server";
...
''',
    "src/components/mir/MIRModal.tsx": r'''"use client";
...
''',
    "README.md": r'''# Enterprise Pharmaceutical MIR Platform
...
ORDERED_SECTIONS = [
    ...
    "Frontend landing page and MIR modal",
    "Accessibility and SEO details",
...
def build_markdown() -> str:
    ...
    failed_response = {
        ...
        "trackingId": None,
        "message": "Please correct the highlighted fields.",
        "errors": ["A valid email address is required.", "Consent must be accepted before submission."],
    }
    ...
    sections: list[str] = [
        ...
        f"## 1. {ORDERED_SECTIONS[0]}\n\n{tree()}",
        f"## 2. {ORDERED_SECTIONS[1]}\n\n```bash\nnpm install\ncp .env.example .env\nnpm run prisma:generate\nnpm run prisma:migrate\nnpm run dev\n```",
        f"## 3. {ORDERED_SECTIONS[2]}\n\n{code_block('prisma/schema.prisma')}",
        f"## 4. {ORDERED_SECTIONS[3]}\n\n{code_block('src/lib/security.ts')}\n\n{code_block('src/lib/validation.ts')}",
        f"## 5. {ORDERED_SECTIONS[4]}\n\n{code_block('src/lib/prisma.ts')}\n\n{code_block('src/app/api/medical-inquiry/route.ts')}",
        f"## 6. {ORDERED_SECTIONS[5]}\n\n{code_block('src/lib/rate-limit.ts')}",
        f"## 7. {ORDERED_SECTIONS[6]}\n\n{code_block('src/lib/email.ts')}",
        f"## 8. {ORDERED_SECTIONS[7]}\n\n{code_block('src/app/page.tsx')}\n\n{code_block('src/components/mir/MIRModal.tsx')}",
        f"## 9. {ORDERED_SECTIONS[8]}\n\n{code_block('src/app/layout.tsx')}\n\nThe modal uses a real dialog role, `aria-modal`, labelled controls, visible validation errors, status messaging, focusable controls, keyboard-compatible inputs, and a duplicate-submit guard. The layout injects JSON-LD for the medical organization and medical information contact point.",
        ...
        f"## 11. {ORDERED_SECTIONS[10]}\n\n{code_block('.env.example')}",
        f"## 12. {ORDERED_SECTIONS[11]}\n\nSuccessful response:\n\n```json\n{json.dumps(successful_response, indent=2)}\n```\n\nValidation response:\n\n```json\n{json.dumps(failed_response, indent=2)}\n```",
...
def write_workspace(target: Path) -> None:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate the pharma MIR golden benchmark response.")
    parser.add_argument("--output", type=Path, help="Write the markdown golden response to a file.")
    parser.add_argument("--write-workspace", type=Path, help="Materialize the embedded project workspace.")
...
def main() -> int:
    ...
    if args.output:
        ...
        print(f"Wrote markdown response to {args.output}")
    else:
        print(markdown)

    if args.write_workspace:
        write_workspace(args.write_workspace)
        print(f"Wrote project workspace to {args.write_workspace}")
